...
2. **Programmatic Gate CLI** (`node scripts/security-gate.mjs`):
   - Computes an overall Security Score (0–100) and Letter Grade (A, B, C, D, F) based on AgentShield findings.
   - Enforces **hard failure** (exit code 1) on secrets/credentials or Grade below threshold (default: Grade B).
   - Generates actionable markdown reports with issue IDs, severity counts, and remediation instructions.
   - Integrated into `/review` (scope: changed) and `/ship` (scope: full project).
